  ...
  return {
    ...
    provider: {
      ...providerMetadataBase({
        ...
        timedOutPid: input.processResult?.timedOutPid,
        killMethod: input.processResult?.killMethod,
        killSucceeded: input.processResult?.killSucceeded,
        taskkillAttempted: input.processResult?.taskkillAttempted,
        taskkillExitCode: input.processResult?.taskkillExitCode,
        taskkillSignal: input.processResult?.taskkillSignal,
        taskkillError: input.processResult?.taskkillError,
        taskkillStdoutPreview: input.processResult?.taskkillStdoutPreview,
        taskkillStderrPreview: input.processResult?.taskkillStderrPreview,
        taskkillStdoutEncoding: input.processResult?.taskkillStdoutEncoding,
        taskkillStderrEncoding: input.processResult?.taskkillStderrEncoding,
        taskkillStdoutHadReplacement: input.processResult?.taskkillStdoutHadReplacement,
        taskkillStderrHadReplacement: input.processResult?.taskkillStderrHadReplacement,
        fallbackKillAttempted: input.processResult?.fallbackKillAttempted,
        fallbackKillSucceeded: input.processResult?.fallbackKillSucceeded,
        fallbackKillError: input.processResult?.fallbackKillError,
        stdoutLengthAtTimeout: input.processResult?.stdoutLengthAtTimeout,
        stderrLengthAtTimeout: input.processResult?.stderrLengthAtTimeout,
        stdoutLengthAtClose: input.processResult?.stdoutLengthAtClose,
        stderrLengthAtClose: input.processResult?.stderrLengthAtClose,
        outputGrewAfterTimeout: input.processResult?.outputGrewAfterTimeout,
        timeoutOverrunMs: input.processResult?.timeoutOverrunMs,
        killFailureSummary: input.processResult?.killFailureSummary,
